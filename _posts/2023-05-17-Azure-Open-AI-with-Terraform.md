...
- [{{ page.title }}](#-pagetitle-)
  ...
  * [Terraform Code Details](#terraform-code-details)
    + [main.tf](#maintf)
...
4. The `cognitive_deployments` local value is generated as a flattened list, combining all the maps generated for each deployment and model
...
_account_id`: Specifies the ID of the cognitive account associated with the deployment, obtained from `each.value.cognitive_account_id`. This is dynamicaly assigned `azurerm_cognitive_account` resource block
